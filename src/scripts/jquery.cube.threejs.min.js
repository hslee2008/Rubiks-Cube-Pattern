...
/*
 * Note: this is the changed version of the original jquery.cube.threejs.js plugin
 * There were code that needed to be changed to make it work with Svelte
 */
...
$.fn.cube = function (options) {
  ...
  options = $.extend(
    ...
    {
      type: 3,
      camera: { x: 50, y: 50, z: 100 },
      size: { width: 60, height: 60 },
      color: [
        0xff0000, 0xff8000, 0xffff00, 0x00ff00, 0x0000ff, 0x7fff00, 0xffffff
      ],
      background: 0xffffff,
      animation: { delay: 250 },
      onTurn: $.noop,
  ...
  _ref.turn = function (move) {
    ...
    switch (move) {
      ...
      case 'u':
        ...
        if (options.type == 3) cubits = f1.concat(f2)
        else cubits = f1
  ...
  _ref.execute = function (moves) {
    ...
    if (!ms) _ref.trigger('next-move')
  }
  _ref.on('next-move', function () {
    var moves = _ref.data('move-stack')
    if (!moves) return
    var move = moves.shift()
    if (!move) moves = null
    _ref.data('move-stack', moves)
    if (move) _ref.turn(move)
    else options.onComplete(_ref)
  })
  function parse(moves) {
    ...
    for (var i = 0; i < m.length; i++) {
      var move = m[i]
      if (move.length == 0 || move.length > 3) continue
      var sane = true
  ...
  function colorFaceTexture(width, height, color) {
    if (colorFaceTexture[color]) return colorFaceTexture[color]
  ...
  function createCubit(point) {
    ...
    if (!createCubit.box)
      createCubit.box = new THREE.BoxGeometry(
        options.cubit.width,
        options.cubit.height,
        options.cubit.height,
        tessellation,
        tessellation,
        tessellation
  ...
  function createCube() {
    ...
    var offset = 0
    if (options.type % 2 == 0) offset = options.cubit.width / 2
  ...
  function orientCubits(cubits) {
    ...
    _pivot.rotation.x = _pivot.rotation.y = _pivot.rotation.z = 0
  }
  function constructor() {
    _scene = new THREE.Scene()
    _camera = new THREE.PerspectiveCamera(
      60,
      _ref.width() / _ref.height(),
      0.1,
      1000
    )
    try {
